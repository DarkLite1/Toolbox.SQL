...
Function Invoke-Sqlcmd2 {
    ...
    .DESCRIPTION
        Runs a T-SQL script. Invoke-Sqlcmd2 runs the whole script and only 
        captures the first selected result set, such as the output of PRINT 
        statements when -verbose parameter is specified.
...
Function Save-TicketInSqlHC {
    ...
    Process {
        Try {
            ...
            $SQLResult = Invoke-Sqlcmd2 @SQLParams -As PSObject -Query "
                SELECT * 
                FROM $Table 
                WHERE PSCode = '$PSCode'"
            ...
            $InsertRow = {
                $null = Invoke-Sqlcmd2 @SQLParams -Query "
                    INSERT INTO $Table 
